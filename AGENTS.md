...
- **작업 지침:** 새로운 API 엔드포인트 생성 규칙, 테스트 코드 작성 템플릿, 상세 프롬프트 등 구체적인 작업 스킬 및 매뉴얼은 `/.Codex/skills.md`를 읽고 지시를 따르세요.
- **언어: 사용자와의 모든 대화와 코드 설명은 반드시 한국어(Korean)로 대답하세요.**
- **작업 종료 이후:** 
  - 파일의 수정 사항을 간략히 요약해 주세요.
  - 새롭게 생성되거나 변경된 코드의 API 엔드포인트와 HTTP 메서드, URL 경로, 간단한 설명을 포함하여 표(Table) 형식으로 출력해 주세요.
  - `docker-compose up --build -d` 명령어를 사용하여 전체 인프라를 재실행해 주세요.